...
**Why:** Happy Hare's GCode API evolves between versions. The exact command for "tell Happy Hare about this spool" has been `MMU_GATE_MAP`, may later include `MMU_SPOOLMAN UPDATE=1`, and could change again. By keeping the actual command in an editable GCode macro, users can adjust the Happy Hare call for their version without touching Python.
